...
let [start_x, end_x] = [
  Math.min(min_w, 500) - 2 - max_h,
  Math.max(max_w, 500) + 2 + max_h,
];
let floor = max_h + 1;
let wall = [...new Array(floor + 1)].map(_ =>
  [...new Array(end_x - start_x + 1)].map(_ => 0)
);
...
const faireTomberSable = () => {
  ...
  while (true) {
    ...
    sand_units++;
    while (true) {
      while (y < floor && wall[y + 1][x] == 0) {
        y++;
      }
      if (y < floor && wall[y + 1][x - 1] == 0) {
        y++;
        x--;
      } else if (y < floor && wall[y + 1][x + 1] == 0) {
        y++;
        x++;
      } else {
        wall[y][x] = 2;
        if (y == 0) return sand_units;
        break;
      }
    }
  }
